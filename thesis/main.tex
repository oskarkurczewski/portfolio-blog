...
SQLite jest domyślnym systemem zarządzania bazą danych dla Strapi. W porównaniu do innych popularnych systemów takich jak MySQL czy PostgresQL, SQLite wyróżnia się tym, że jest biblioteką i nie wymaga oddzielnego procesu --- zapis i odczyt wykonywany jest po prostu za pomocą pliku bazy danych \cite{sqlite}. Dzięki temu jego ogromną zaletą są brak konieczności instalacji oraz konfiguracji bazy danych, a co za tym idzie --- łatwość użytkowania. 
...
W tym rozdziale przedstawione zostaną założenia i wymagania (z podziałem na funkcjonalne i niefunkcjonalne) dotyczące projektu portfolio i blogu z systemem komentowania. W podrozdziale \ref{realizacja} zaprezentowane będą również najważniejsze aspekty implementacji tworzonej strony internetowej.
